...
# --------------------------
# 8. Compute Nearest Neighbors
# --------------------------

print("Computing nearest neighbors for each PCA component...")

# Extract PCA components (eigenvectors)
pca_components = pca.components_  # Shape: [n_components, embedding_dim]

# Normalize PCA components and classification weights for cosine similarity
pca_components_norm = normalize(pca_components, axis=1)  # Shape: [n_components, embedding_dim]
classification_weights_norm = normalize(classification_weights, axis=1)  # Shape: [1000, embedding_dim]

# Compute cosine similarity between each PC and each class vector
# Cosine similarity = dot product of normalized vectors
similarity_matrix = np.dot(pca_components_norm, classification_weights_norm.T)  # Shape: [n_components, 1000]

# For each PC, find the class with the highest similarity
nearest_class_indices = np.argmax(similarity_matrix, axis=1)  # Shape: [n_components]

# Map class indices to class labels
nearest_class_labels = [labels.get(str(idx), "Unknown") for idx in nearest_class_indices]

# --------------------------
# 9. Print Nearest Classes for Each PC
# --------------------------

print("\nNearest ImageNet Class for Each Principal Component:")
for pc_num, class_label in enumerate(nearest_class_labels, start=1):
    print(f"PC{pc_num}: Nearest class - {class_label}")